...
#define uart_txp_wrap_target 0
#define uart_txp_wrap 3
#define uart_txp_pio_version 1

static const uint16_t uart_txp_program_instructions[] = {
            //     .wrap_target
    0x9fa0, //  0: pull   block           side 1 [7] 
...
#if !PICO_NO_HARDWARE
static const struct pio_program uart_txp_program = {
    .instructions = uart_txp_program_instructions,
    .length = 4,
    .origin = -1,
    .pio_version = uart_txp_pio_version,
#if PICO_PIO_VERSION > 0
    .used_gpio_ranges = 0x0
#endif
};

static inline pio_sm_config uart_txp_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + uart_txp_wrap_target, offset + uart_txp_wrap);
    sm_config_set_sideset(&c, 2, true, false);
    return c;
}
